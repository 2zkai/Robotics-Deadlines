      ...
      <el-table-column>
        <template slot-scope="scope">
          <div :class="{ 'conf-fin': scope.row.status === 'FIN' }">
            ...
            <el-row><el-tag size="mini" type="" effect="plain">{{scope.row.sub}}</el-tag> <el-tag size="mini" type="" effect="plain">Tier {{scope.row.rank}}</el-tag> <el-tag v-if="scope.row.corerank!=='N'" size="mini" type="" effect="plain">CORE {{scope.row.corerank}}</el-tag> <el-tag v-if="scope.row.thcplrank!=='N'" size="mini" type="" effect="plain">THCPL {{scope.row.thcplrank}}</el-tag> <span style="color: #409eff" v-show="scope.row.comment"><b>NOTE:</b> {{scope.row.comment}}</span></el-row>
  ...
  <!--          <el-row>subscribe</el-row>-->
    ...
    <el-row style="padding-top: 8px">
      <div style="float: left; color: #666666;font-size: 12px;">
        <div>robotics-deadlines is based on <a href="https://github.com/ccfddl/ccf-deadlines">ccf-deadlines</a>.</div>
        <div style="padding-top: 3px">If you find it useful, consider giving it a star on <a href="https://github.com/your-username/robotics-deadlines">GitHub</a>.</div>
      </div>
      <div style="float: right">
        <el-pagination
            background
...
export default {
  ...
  data() {
    return {
      ...
      timeZone: '',
      utcMap: new Map(),
      rankoptions: {'A': 'Tier A', 'B': 'Tier B', 'C': 'Tier C', 'N': 'Others'},
      typesList: [],
      rankList: [],
      cachedLikes: [],
      cachedRanks: [],
  ...
  methods: {
    ...
    getAllConf() {
      this.$http.get(this.publicPath + 'conference/conf.yml').then(response => {
        let allDoc = yaml.load(response.body)
        allDoc.forEach(item => {
          item.confs.forEach(conf => {
            conf.description = item.description
            conf.dblp = item.dblp
            conf.title = item.title
            conf.sub = item.sub
            conf.subname = this.typeMap.get(item.sub)
            conf.rank = item.rank.ccf
            conf.corerank = item.rank.core? item.rank.core : 'N'
            conf.thcplrank = item.rank.thcpl ? item.rank.thcpl : 'N'
            conf.displayrank = 'Tier ' + item.rank.ccf
            conf.timeline = item.timeline
            conf.deadline = item.timeline[item.timeline.length - 1].deadline
            conf.abstract_deadline = item.timeline[item.timeline.length - 1].abstract_deadline
            conf.comment = item.timeline[item.timeline.length - 1].comment
            conf.ddls = item.ddls
            conf.status = item.status
            conf.acc = item.acc
            conf.link = item.link
            conf.timezone = item.timezone
            conf.year = item.year
            conf.place = item.place
            conf.localDDL = item.localDDL
            conf.originDDL = item.originDDL
            conf.remain = item.remain
            this.allconfList.push(conf)
            this.allconfMap.set(conf.title + conf.year, conf)
          })
        })
